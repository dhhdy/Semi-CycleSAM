...
class KDLoss(nn.Module):
    ...
    def forward(self, out_s, out_t):
        out_s = out_s.permute(0,2,3,4,1).reshape(-1, 1)
        out_t = out_t.permute(0,2,3,4,1).reshape(-1, 1)
        loss = (
...
class RKDLoss(nn.Module):  # 多类别损失
    """Relational Knowledge Disitllation, CVPR2019"""

    def __init__(self, w_d=300, w_a=600):
    ...
    def forward(self, f_s, f_t):
        ...
        # RKD distance loss
        with torch.no_grad():
            t_d = self.pdist(teacher, squared=False)  #693
