...
all_flair = glob.glob("/data1/data/lung/I_IIIA_LUAD/GD/imagesTr/*.nii.gz")
for p in all_flair:
    ed = p.split('/')[-1].split('.')[0]
    fr = ed.split('_')[0]
    data = sitk.GetArrayFromImage(sitk.ReadImage(p))
    lab = sitk.GetArrayFromImage(sitk.ReadImage(p.replace(ed, fr).replace("imagesTr", "labelsTr")))
    ...
    sitk.WriteImage(sitk.GetImageFromArray(
        img), "/data1/data/lung/I_IIIA_LUAD/imagesTr/{}".format(uid))
    sitk.WriteImage(sitk.GetImageFromArray(
        lab), "/data1/data/lung/I_IIIA_LUAD/labelsTr/{}".format(uid))
